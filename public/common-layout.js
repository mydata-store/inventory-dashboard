...
function itemMatches(item,key){if(item.match&&item.match.some(x=>key.includes(x)))return true;return item.href&&(key===item.href||key.startsWith(item.href))}
function toggleSidebarGroup(groupId){
 document.querySelectorAll(".nav-group").forEach(group=>{if(group.id!==groupId)group.classList.remove("open")});
 document.getElementById(groupId)?.classList.toggle("open");
}
function buildSidebar(){
 const host=document.getElementById("appSidebar");if(!host)return;
 const key=currentPageKey();let html=`<aside class="app-sidebar"><div class="app-brand"><div class="app-brand-icon">◆</div><div><h2>Inventory Store</h2><p>Management System</p></div></div><nav class="app-nav">`;
 APP_LAYOUT.menu.forEach(item=>{
  if(item.children){
   ...
   item.children.forEach(child=>html+=`<a class="${itemMatches(child,key)?"active":""}" href="${child.href}">${child.label}</a>`);
   html+=`</div></div>`;
  }else html+=`<a class="${itemMatches(item,key)?"active":""}" href="${item.href}">${item.label}</a>`;
 });
