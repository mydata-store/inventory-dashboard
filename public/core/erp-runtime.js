(function(){
'use strict';
const ERP={version:'41.0.0',modules:new Map(),pages:new Map(),state:{online:navigator.onLine,booted:false,settings:{}},events:new EventTarget()};
...
ERP.boot=async()=>{if(ERP.state.booted)return ERP;ERP.state.booted=true;document.documentElement.dataset.erpBoot='starting';
 for(const id of ['settings','offline','backup','host','theme','profile','shell']){const m=ERP.modules.get(id);if(m?.init)await ERP.safe(id,()=>m.init(ERP));}
